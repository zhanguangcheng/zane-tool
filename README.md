...
#### Excel 批处理
- 规则表：表头精确替换 / 数据子串替换 / 条件清空或删除行
- 规则 JSON 导入导出，多文件批处理（仅处理首个工作表）
- 输出默认同目录 `<原名>_batch.xlsx`，绝不覆盖源文件

#### PDF 工具
- **合并**：多文件列表可拖动排序，按顺序拼接
- **拆分 / 提取**：按页码范围提取到单文件、每页一个文件、每 N 页一个文件
- **页面编排**：页序表达式删除/重排页面，支持相对/绝对角度旋转
- **加密 / 解密**：AES-256 加密，可设置打印/复制/修改权限；解密移除保护
- **压缩 / 优化**：线性化、对象流、流重压缩、图片优化，显示前后体积对比
- **信息 / 元数据**：PDF 版本、页数、加密状态、标题/作者/时间等
- 基于 qpdf 子进程；支持加密源文件「打开密码」，输出不覆盖源文件
...
- mkcert.exe 放置于项目根目录（HTTPS 证书功能需要）
- qpdf.exe / qpdf30.dll 放置于项目根目录（PDF 工具需要）
